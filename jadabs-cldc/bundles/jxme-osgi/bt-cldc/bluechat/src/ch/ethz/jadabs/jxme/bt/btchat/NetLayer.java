/*
 * $Id: NetLayer.java,v 1.1 2004/11/10 10:28:13 afrei Exp $
 */
...
/**
 * Main class for handling bluetooth connectivity.
 * 
 * <p>Description:
 * This is the main class for handling bluetooth connectivity and device/service
 * discovery process. This class does many things, including - search for
 * bluetooth devices (query()) - create a local BlueChat server and register it
 * with bluetooth (run()) - search for remote BlueChat services using
 * searchServices() - handle incoming connection request from remote BlueChat -
 * establish connection to remote BlueChat.</p>
 * 
 * <pre>
...
public class NetLayer implements Runnable
{
    ...
    private Hashtable serviceRecordToEndPoint = new Hashtable();
    ...
    public void init(String name, BTListener callback)
    {
        ...
        try
        {
            ...
            // singleton
            localDevice.setDiscoverable(DiscoveryAgent.GIAC); // set Discover
            // mode to GIAC
            agent = localDevice.getDiscoveryAgent(); // obtain reference to
            // singleton
    ...
    public EndPoint findEndPointByRemoteDevice(RemoteDevice rdev)
    {
        ...
        {
            EndPoint endpt = (EndPoint) endPoints.elementAt(i);
            if (endpt.remoteDev.equals(rdev)) { return endpt; }
        }
    ...
    public EndPoint findEndPointByTransId(int id)
    {
        for (int i = 0; i < pendingEndPoints.size(); i++)
        {
            ...
            if (endpt.transId == id) { return endpt; }
        }
        return null; // not found, return null
    }
    ...
    public void run()
    {
        ...
        try
        {
            ...
            server = (StreamConnectionNotifier) Connector.open("btspp://localhost:" + uuid.toString()
                    + ";name=BlueChatApp");
        ...
        while (!done)
        {
            try
            {
                ...
                // this message is to inform user that the server is up and
                // ready
    ...
    private static void log(String s)
    {
        ...
        // "N" means NetLayer
        if (ChatMain.isDebug) ChatMain.gui_log("N", s);

    }

    /**
     * Internal discovery listener class for handling device & service discovery
     * events.
     * 
     * @author Ben Hui
     * @version 1.0
    ...
    class Listener implements DiscoveryListener
    {
        ...
        public void deviceDiscovered(RemoteDevice remoteDevice, DeviceClass deviceClass)
        {
            ...
            // only device of SERVICE_OBJECT_TRANSFER will be considered as
            // candidate device
            // because in our BlueChat service, we explicitly set the service
            // class to
            // SERVICE_OBJECT_TRANSFER. see the run() method
            //      if ( (deviceClass.getServiceClasses() & SERVICE_OBJECT_TRANSFER)
            // != 0 )
            //      {
            try
            {
            ...
            //      } else
            //      {
            //        log("found device that is not Object Transfer Service, ignore
            // this device...");
            //      }
        }
         ...
         * to search for BlueChat services. We loop through all the pending
        ...
        /**
         * A service is discovered from a remote device. When a BlueChat service
         * is discovered, we establish a connection to this service. This signal
         * joining the existing virtual chat room.
         * 
         * @param transId
         * @param svcRec
         */
         ...
         * @param respCode response (result) code of search
         */
        public void serviceSearchCompleted(int transID, int respCode)
        {
            ...
            else if (respCode == SERVICE_SEARCH_DEVICE_NOT_REACHABLE) log("SERVICE_SEARCH_DEVICE_NOT_REACHABLE");
            ...
            for (Enumeration records = serviceRecordToEndPoint.keys(); records.hasMoreElements();)
            {
                try
                {
                    ...
                    // We make an assumption that the first service is BlueChat.
                    // In fact, only one
                    // service record will be found on each device.
                    // Note: we know the found service is BlueChat service
                    // because we search on specific UUID,
                    ...
                    // retrieve the pending EndPoint and initialize the
                    ...
                    if (endpt != null)
                    {
                        ...
                        // once a EndPoint established, the BlueChat client is
        ...
        public void run()
        {
            ...
            for (int i = 0; i < pendingEndPoints.size(); i++)
            {
                ...
                try
                {
                    ...
                    //
                    // searchServices return a transaction id, which we will
                    // used to
                    // identify which remote device the service is found in our
                    // callback
                    // listener (class Listener)
                    //
                    // note: in theory, only one runtine instance of Listener is
                    // needed
                    // to handle all discovery callback. however, there is a bug
                    // in rococo
                    // simualtor that cause callback fails with one instance of
                    // used
                    // so we make a new Listener for every searchServices()
                    endpt.transId = agent.searchServices(null // null to
                            // indicate
                            // retrieve
                            // default
                            // attributes
                            , new UUID[] { uuid} // BlueChat service UUID
                            // SerialPort
                            , endpt.remoteDev, new Listener());

                    // wait until the above service discovery is completed
                    // because N6600 cannot handle more than one service
                    // discovery
