/*
 * $Id: Util.java,v 1.1 2004/11/10 10:28:13 afrei Exp $
 */
...
/**
 * A utility class to dump JABWT object contents.
 * 
 * <p>
 * Description:
 * </p>
...
public class Util
{
    ...
    private static void log(String s)
    {
        ...
        // "U" means Util class
        if (ChatMain.isDebug) ChatMain.gui_log("U", s);
    ...
    public static void printRemoteDevice(RemoteDevice dev, DeviceClass devClass)
    {
        try
        {
            ...
            log("Auth: " + dev.isAuthenticated() + " Encrypted: " + dev.isEncrypted() + " Trusted: "
                    + dev.isTrustedDevice());

            if (devClass != null)
            {
                log("MajorDevice:" + majorToName(devClass.getMajorDeviceClass()));
                log("MinorDevice:" + minorToName(devClass.getMajorDeviceClass(), devClass.getMinorDeviceClass()));
                log("ServiceClass:");
    ...
    public static String idToName(int id)
    {
        ...
        {
            /**
             * @todo why the spec say it is GroupID, IpSubnet and
             *       VersionNumberList as well?
             */
            return "GroupID";
        } else if (id == 0x0201)
        {
     ...
     * @param m
     *            minor device class
     * @return String representing device class
     */
    ...
    public static String[] majorServiceToName(int d)
    {
        ...
        if ((d & 0x2000) > 0) v.addElement("LimitedDiscoverableMode");
        if ((d & 0x10000) > 0) v.addElement("Positioning");
        if ((d & 0x20000) > 0) v.addElement("Networking");
        if ((d & 0x40000) > 0) v.addElement("Rendering");
        if ((d & 0x80000) > 0) v.addElement("Capturing");
        if ((d & 0x100000) > 0) v.addElement("ObjectTransfer");
        if ((d & 0x200000) > 0) v.addElement("Audio");
        if ((d & 0x400000) > 0) v.addElement("Telephony");
        if ((d & 0x800000) > 0) v.addElement("Information");
    ...
    /*
     * This function requires javax.obex which is not availabe on Nokia Series60
     * Concept SDK beta 0.3 public static void printObexHeaderSet( HeaderSet h ) {
     * try { log("Print OBEX Header"); int[] ids = h.getHeaderList(); for (int i =
     * 0; i < ids.length; i++) { log("ID[" + ids[i] + "]: " +
    ...
    public static boolean isContainsUUID(ServiceRecord r, UUID uuid)
    {
        ...
        for (int i = 0; i < ids.length; i++)
        {
            ...
            if (isContainsUUID(e, uuid)) return true;
        }
        return false;
    ...
    private static boolean isContainsUUID(DataElement e, UUID uuid)
    {
        ...
        {
            if (e.getValue().equals(uuid)) return true;
