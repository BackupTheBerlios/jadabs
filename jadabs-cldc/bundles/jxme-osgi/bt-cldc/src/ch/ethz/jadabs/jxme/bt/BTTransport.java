...
{   
    ...
    /** Protocol Service UUID code for PublicBrowseGroup
    ...
    /** 
     * Service Attribute identifier code for BrowseGroupList 
     * (see Bluetooth Assigned Numbers document
     *  https://www.bluetooth.org/foundry/assignnumb/document/service_discovery) 
     */
    ...
    /** only if authenticated: true if Bluetooth connection requires authorization by
     *  the user. 
     */
    ...
    /** 
     * Hashtable containing client connections to this BT Service. It 
     * stores EndpointAddress instances as keys and BTConnnection instances
     * as values.
     */ 
     ...
     * 
     * @param startService If set to true this host sets up the service.
     * It is thus becomes a slave peer that rendez-vous peers can connect to.
     * If false no service entry is created, this host thus is a rendez-vous
     * peer that can connect to other non-rendezvous peers while taking 
     * the Bluetooth Master role   
     *  
     */
     ...
     * @param myURL Endpoint-Address of this BT-Interface (argument is ignored, 
     *         use <code>null</code> instead, it will be automatically determined on the 
     *         BT-MAC address of the BT-Interface within this method)
     * @param listener EvenpointService that will be notified if a new message 
     *         is received.
    ...
    public void init(EndpointAddress myURL, Listener listener) throws IOException
    {
        ...
        // Set port number =0 since we do not know how to handle portnumbers in BT
        myEndpointURL = new EndpointAddress("btspp", localDevice.getBluetoothAddress());
        
        agent = localDevice.getDiscoveryAgent();
        serviceURL = "btspp://localhost:"+JXME_BT_UUID.toString()+";authenticate="+authenticate
                    +";authorize="+authorize+";encrypt="+encrypt+";name="+SERVICE_NAME;
     ...
     * @param destURI destination the message has to be sent to 
     * @throws IOException if the specified destination is not listed in 
     *         the connection table or if the underlying <code>OutputStream</code>
    ...
    {
        if (!connections.containsKey(destURI)) {
            throw new IOException("Destionation "+destURI+" not in connections table.");
        }
        BTConnection connection = (BTConnection)connections.get(destURI);
        ...
        // send byte array
        connection.sendMessage(bout.toByteArray(), 
                               connection.getRemoteBTAddress(), false);        
    }
    ...
    public void propagate(Message message) throws IOException
    {
        ...
        while (enum.hasMoreElements()) {
            ...
            try {                
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("cannot forward multicast message to "+
                              connection.getRemoteBTAddress());
                }
            }
        }
        if (rememberException)   {
            throw new IOException("The message could not be forwarded to " +
            		"at least one client.");            
        }
    }
    ...
    /**
     * Callback from BTConnection that informs BTtransport that 
     * a new message has arrived that should be delivered to the upper layers. 
     * @param receiver BT device address of receiver (i.e. this host)
     * @param sender BT device address of sender 
     * @param message byte array that contains the message itself
     * @param multicast flag indiciating if this is a broadcast message and 
     *        has to be relayed further.
     */
    void deliverMessage(String receiver, String sender, byte[] message, boolean
                         multicast) 
    {
            ...
            while (enum.hasMoreElements()) {
                ...
                if (!destination.equals(sender)) {
                    ...
                    try {
                        ...
                        LOG.error("Cannot forward multicast message to "+
     ...
     * @param connection refernence to the object that encapsulates this connection
     */
    public void addConnection(BTConnection connection) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("adding connection to "+connection.getRemoteBTAddress()
                     +" into pool.");
        }
     ...
     * @param btAddress 12-Byte hex-String Bluetooth Address
    ...
    /**
     * Callback from BTConnection where BTTransport is informed that 
     * a connection to a BT client was closed
     * @param closedConn reference to the closed connection
     */
    ...
    /**
     * This class implements the read that accepts new connections to 
     * clients (i.e. Rendez-Vous Peers). 
     * 
     * @author Ren&eacute; M&uuml;ller
     */
            ...
            while (!aborting) {
                try {
                    ...
                    BTConnection connection = new BTConnection(conn, 
                            RemoteDevice.getRemoteDevice(conn), BTTransport.this);
