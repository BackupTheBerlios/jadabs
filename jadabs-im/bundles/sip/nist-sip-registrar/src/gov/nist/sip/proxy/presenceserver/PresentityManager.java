...
import gov.nist.sip.proxy.presenceserver.pidfparser.PresenceTag;
import gov.nist.sip.proxy.presenceserver.pidfparser.TupleTag;
import gov.nist.sip.proxy.presenceserver.pidfparser.XMLcpimParser;
...
public class PresentityManager implements Runnable
{
    ...
    /***************************************************************************
     * A list of all registered notifiers. The objects are only referenced to
     * from this hashtable, and removing them from it should delete them
     **************************************************************************/
    ...
    public PresentityManager(PresenceServer presenceServer)
    {
        ...
        notifiers = ProxyActivator.getUserDB().getUserMap();
    ...
    public synchronized int processSubscribe(Request request, Dialog dialog, int expires)
    {
        ...
        if (subscriber != null)
        {
            ...
            } else
            {
                ...
                //to this notifier. Possibly the subscriberId (dialogId) has
                // been reused
                //don't send notify
        ...
        initialNotifyQueue.add((Object) subscriber);
        ProxyDebug.println(" PM.processSubscribe finished, added subscriber to NotifyQueue: "
                + initialNotifyQueue.toString());
    ...
    private int processResourceListSubscribe(Request request, Dialog dialog, int expires)
    {
        ...
        } else
        {
            ...
            if (resourceListFile.length() < 1) { return Response.NOT_FOUND; }
                ...
                synchronized (notifier)
                {
                    if (notifier != null)
                    {
                        ...
                        if (expires > 0)
                            ...
                            notifier.addSubscriber(resourceList.getSubscriberURL(), resourceList.getSubscriberURL(), //resourcelists
                                                                                                                     // have
                                                                                                                     // no
                                                                                                                     // dialog,
                                                                                                                     // URI
    ...
    /**
     * Extra sendNotify for the special case when the initial notify of a new
     * subscripion is to be send. This is required since the response to the
     * subscription must be sent before the Notify and fetchers and terminated
     * subscriptions are lost when the processSubscribe method finishes.
     */
    ...
    /**
     * Responsible for updating internal Notifier state and updating internal
     * states of it's subscribers. If expires!=0, creates a Notify object else
     * all subscribers are notified and the Notifier object is deleted.
     * <p />
     * TODO
     * <ul>
     * <li>Add fields for contacts, accept-headers and allow-events in Notify
     * object and constructor</li>
     * </ul>
     */
    public void processRegister(String notifierKey, int expires, String contact)
    {
        synchronized (notifiers)
        {
            ...
            if ((notifier != null) && (expires == 0))
            {
                ...
                //Update subscribers
                //This should be done if the subscribers support Partial
                // presence
                /*
                 * Collection notifiersSubscriberKeys =
                 * notifier.getSubscribers(); Iterator it =
                 * notifiersSubscriberKeys.iterator(); String text = " <removed>
                 * <t_id>" + tupleId + " </t_id> </removed>"; while
                 * (it.hasNext()) { String dialogId = (String)it.next();
                 * Subscriber subscriber =
                 * (Subscriber)subscribers.get(dialogId);
    ...
    /**
     * Responsible for updating Notifier internal state and also updating all
     * it's subscribers accordingly. <br>
     * 
     * Todo:
     * <ul>
     * <li>Verification</li>
     ...
     * 403 (Forbidden), 489 (Bad Event), 415 (Unsupported Media Type), 200 (OK)
    ...
    public synchronized int processPublish(Request request)
    {
        ...
        if ((eventHeader == null) || !(notifier.hasEvent(eventHeader.getEventType()))) { return Response.BAD_EVENT; }
    ...
    public synchronized int processNotify(Request request, Dialog dialog)
    {
        ...
        SubscriptionStateHeader subscriptionStateHeader = (SubscriptionStateHeader) request
        ...
        //if subscription-state was "terminated", check for retry-parameter and
        // behave accordingly

        if (responseCode == Response.OK)
        {
            //update vs's subscribers body (a single virtual subscription state
            // cannot effect a whole subscriber)
    ...
    /**
     * @param notifierURI
     *            cleaned SipURI as found in a SUBSCRIBE To-header
     */
    ...
    /**
     * Subscribers come in many flavours. This method determines which
     * Subscriber subclass should be created and creates it.
     * 
     * @return A type-casted Subscriber object or null if the headers in the
     *         subscription were ambigous or insufficient.
     */
    private Subscriber createSubscriber(Request request, String subscriberKey, Dialog dialog, int expires)
    {
        ...
        } else if (event.equalsIgnoreCase("watcher.presence"))
        {
            //get notifier
            //get winfo file
            //SubscriberWinfo subscriberWinfo = new
            // SubscriberWinfo(subscriberKey, dialog, expires, winfoFile);
    ...
    private boolean isResourceListSubscriber(Request request)
    {
        ...
        } else
        {
            ...
            while (supportedHeadersIterator.hasNext())
            {
                ...
                if (header.indexOf("resourceList") > 0) { return true; }
            }
        }
        return false;
    }
        ...
        while (this.isRunning)
        {
            ...
            synchronized (subscribers)
            {
                ...
                while (sit.hasNext())
                {
                    ...
                    //collect max expiresTime for possible server
                    // resubscription
