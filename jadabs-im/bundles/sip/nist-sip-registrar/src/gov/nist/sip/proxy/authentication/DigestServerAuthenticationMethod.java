...
public class DigestServerAuthenticationMethod implements AuthenticationMethod
{
    ...
    /**
     * Initialize -- load password files etc. Password file format is
     * name:authentication domain:password
     * 
     * @param pwFileName
     *            is the password file name.
     * @param Exception
     *            is thrown when the password file is bad.
     */
    public void initialize(String pwFileName)
    {
        try
        {
            ...
            if (def != null)
                DEFAULT_REALM = def;
            ProxyDebug.println("DEBUG, DigestAuthenticationMethod, initialize()," + " the realm is:" + DEFAULT_REALM);
    ...
    public boolean doAuthenticate(String user, AuthorizationHeader authHeader, Request request)
    {
        ...
        ProxyDebug.println("DEBUG, DigestAuthenticateMethod, doAuthenticate(): " + "Trying to authenticate user: "
                + username + " for " + " the realm: " + realm);

        //String password = (String) passwordTable.get(username + "@" + realm);
        String password = gov.nist.sip.proxy.ProxyActivator.getUserDB().getPassword(username);
        if (password == null)
        {
            ProxyDebug.println("DEBUG, DigestAuthenticateMethod, doAuthenticate(): "
        ...
        ProxyDebug.println("DEBUG, DigestAuthenticateMethod, doAuthenticate(): " + "we have to compare his response: "
