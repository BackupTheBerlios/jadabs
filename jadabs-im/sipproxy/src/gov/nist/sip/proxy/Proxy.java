...
/*****************************************************************************/            
		...
		if (ProxyDebug.debug) 
			ProxyDebug.info("Incoming request Register");
		...
		(request.getMethod().equals(Request.SUBSCRIBE))) {
	     	ProxyDebug.info("Incoming request Subscribe");
	     ...
	     if ( isPresenceServer() && (request.getMethod().equals
			(Request.NOTIFY) )) { 
	     	ProxyDebug.info("Incoming request Notify");
	    ...
	    if ( isPresenceServer() && ( request.getMethod().equalsIgnoreCase("PUBLISH"))) {
		 ...
		 ProxyDebug.info("Incoming request Publish");
	  ...
	  if (request.getMethod().equals(Request.BYE) ) {
		...
		("Proxy, processRequest(), the registrar cannot help"+
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////
            // Forward to gw
            // TODO check special header ??
            ////////////////////////////////////////////////////////////////////////////////////////////////
            if (true) {
            	URI gw = addressFactory.createURI("sip:"+System.getProperty("jadabsIM.sip_smtp_gateway"));
            	targetURIList.add(gw);
  ...
  /** Stop the proxy, this method has to be called after the start method
    ...
    public static void main(String args[]) {
    	userDB = new UserDB(System.getProperty("jadabsIM.cayenne_config_file"));
		args = new String [2];
		args[0] = "-cf";
		args[1] = System.getProperty("jadabsIM.sip_config_file");
        try{
            // the Proxy:
	    if (args.length == 0)  {
