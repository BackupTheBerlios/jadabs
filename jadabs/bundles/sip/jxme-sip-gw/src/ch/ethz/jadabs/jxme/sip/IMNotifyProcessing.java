...
import ch.ethz.nist.sip.instantmessaging.presence.cpimparser.NoteTag;
import ch.ethz.nist.sip.instantmessaging.presence.cpimparser.TupleTag;
import ch.ethz.nist.sip.instantmessaging.presence.cpimparser.XMLcpimParser;
import ch.ethz.nist.sip.instantmessaging.presence.pidfparser.AddressTag;
import ch.ethz.nist.sip.instantmessaging.presence.pidfparser.AtomTag;
import ch.ethz.nist.sip.instantmessaging.presence.pidfparser.MSNSubStatusTag;
import ch.ethz.nist.sip.instantmessaging.presence.pidfparser.XMLpidfParser;
...
public class IMNotifyProcessing
{
    ...
    public void processNotify(Request request, ServerTransaction serverTransaction)
    {
        try
        {
             ...
             * Dialog dialog=presentity.getDialog(); if (dialog==null) {
             * DebugIM.println("ERROR, processNotify(), PB to retrieve the
             * dialog"); return; }
             */

            //According to RFC3265 the ExpiresHeader should not exist in a
            // NOTIFY
            // so it should go. Leaving it for backwards compability reasons for
            // now
            if ((expiresHeader != null && expiresHeader.getExpires() == 0)
                    || (subscriptionStateHeader != null && subscriptionStateHeader.getState().equalsIgnoreCase(
            ...
            } else
            {
                ...
                if (text != null && !text.trim().equals(""))
                {
                    ...
                    try
                    {
                        ...
                        DebugIM
                                .println("DEBUG, IMNotifyProcessing, process(), the XML body format" + " is: "
                                        + xmlType);
                        String status = null;
                        if (xmlType.equals("xpidf+xml"))
                        {
                            xmlPidfParser.parsePidfString(text);
                            ch.ethz.nist.sip.instantmessaging.presence.pidfparser.PresenceTag presenceTag = xmlPidfParser
                                    .getPresenceTag();
                            if (presenceTag == null)
                        ...
                        } else if (xmlType.equals("pidf+xml"))
                        {
                            ...
                            ch.ethz.nist.sip.instantmessaging.presence.cpimparser.PresenceTag presenceTag = xmlCpimParser
                                    .getPresenceTag();
                            if (presenceTag == null)
                                DebugIM.println("ERROR: The presence Tag is null!!!");
                            else
    ...
    public void sendNotify(Response okSent, String body, Dialog dialog)
    {
        try
        {
            ...
            if (body == null)
            {
                ...
                request = messageFactory.createRequest(requestURI, "NOTIFY", callIdHeader, cseqHeader, fromHeader,
            ...
            } else
            {
                ...
                request = messageFactory.createRequest(requestURI, "NOTIFY", callIdHeader, cseqHeader, fromHeader,
                        toHeader, viaList, maxForwardsHeader, contentTypeHeader, body);
