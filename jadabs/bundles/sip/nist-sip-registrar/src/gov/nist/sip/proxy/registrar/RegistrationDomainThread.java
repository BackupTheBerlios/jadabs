    ...
    public void sendDomainRegistration()
    {
        try
        {
            ...
            if (domain.hostPort != null)
                ProxyDebug.println("RegistrationDomainThread, sendDomainRegistration(), sending REGISTER"
            ...
            // Call-ID:
            callIdCounter++;
            CallIdHeader callIdHeader = headerFactory.createCallIdHeader("nist-sip-proxy-register-callId"
                    + callIdCounter);
            ...
            if (clientTransaction == null)
            {
                ProxyDebug.println("RegistrationDomainThread, sendDomainRegistration(), "
                        + " ERROR, the client transaction is null for the" + " request" + request);
                return;
            }
            ...
            ProxyDebug.println("RegistrationDomainThread, sendDomainRegistration()," + " REGISTER sent:\n" + request);
            ProxyDebug.println();
        } catch (Exception ex)
        {
            ex.printStackTrace();
