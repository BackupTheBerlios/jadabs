 ...
 *         (convert to simulation) <br/>Henrik Leion: Some changes in how
 *         SUBSCRIBE and NOTIFY are processed. <br/><a href=" {@docRoot}
...
public class Proxy implements SipListener
{
    ...
    {
        return configuration.hasDomain(domainAddress) || registrar.hasRegistration("sip:" + domainAddress);
    }
    ...
    public Proxy(String confFile) throws Exception
    {
        ...
        } else
        {
            try
            {
                ...
                ProxyConfigurationHandler handler = new ProxyConfigurationHandler(confFile);
                configuration = handler.getConfiguration();
                if (configuration == null || !configuration.isValidConfiguration())
                {
                    System.out.println("ERROR: the configuration file is not correct!" + " Correct the errors first.");
                    throw new Exception("ERROR: the configuration file is not correct!" + " Correct the errors first.");
                } else
                {
            ...
            {
                System.out.println("ERROR: exception raised while initializing the proxy");
                ex.printStackTrace();
                throw new Exception("ERROR: exception raised while initializing the proxy");
            }
        }
    }
    ...
    public void processRequest(RequestEvent requestEvent)
    {
        ...
        try
        {
            ...
            if (!requestValidation.validateRequest(sipProvider, request, serverTransaction))
            {
                ...
                // processed!
                if (ProxyDebug.debug)
                    ProxyDebug.println("Proxy, processRequest(), the request has not been"
                            + " validated, so the request is discarded " + " (an error code has normally been"
                            + " sent back)");
                return;
            }
            ...
            if (request.getMethod().equals(Request.ACK))
            {
                ...
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("Proxy, processRequest(), " + "the request is an ACK"
            ...
            if (serverTransaction == null)
            {
                ...
                if (method.equals(Request.INVITE) || method.equals(Request.SUBSCRIBE))
                {
                    try
                    {
                        ...
                        TransactionsMapping transactionsMapping = (TransactionsMapping) serverTransaction.getDialog()
                                .getApplicationData();
                    ...
                    {
                        if (ProxyDebug.debug)
                            ProxyDebug.println("Proxy, processRequest(), this request"
                ...
                if (routes.hasNext())
                {
                    ...
                    if (sipStack.getIPAddress().equals(host))
                    {
                        ...
                        while (lps != null && lps.hasNext())
                        {
                            ...
                            {
                                if (ProxyDebug.debug)
                                    ProxyDebug.println("Proxy, processRequest()," + " we remove the first route form "
                                            + " the RouteHeader;" + " it matches the proxy");
                                routes.remove();
                                break;
                            }
                        }
                    ...
                    if (configuration.hasDomain(requestSipURI.getMAddrParam()))
                    {
                        if (ProxyDebug.debug)
                            ProxyDebug.println("Proxy, processRequest(),"
                                    ...
                                    + " we remove the mAddr parameter from the original" + " request");
            ...
            if (requestURI.isSipURI())
            {
                ...
                if (requestSipURI.getMAddrParam() != null)
                {
                    ...
                    requestForwarding.forwardRequest(targetURIList, sipProvider, request, serverTransaction, true);
                ...
                if (!configuration.hasDomain(requestSipURI.getHost()))
                {
                    ...
                    if (registrar.hasDomainRegistered(request))
                    {
                        ...
                        if (targetURIList != null && !targetURIList.isEmpty())
                        {
                            ...
                            // 4. Forward the request statefully:
                            requestForwarding.forwardRequest(targetURIList, sipProvider, request, serverTransaction,
                                    true);
                            return;
                    ...
                    requestForwarding.forwardRequest(targetURIList, sipProvider, request, serverTransaction, false);
            ...
            if (isPresenceServer() && (request.getMethod().equals(Request.SUBSCRIBE)))
            {
                ...
                } else
                {
                    ...
                    if (targetURIList == null)
                    {
                        ...
                        ProxyDebug.println("Proxy: received a Subscribe request to "
                                + " a user in our domain that was not found, " + " responded 404");
                    ...
                    {
                        ProxyDebug.println("Trying to forward subscribe to " + targetURIList.toString() + "\n"
                                + request.toString());
                        requestForwarding.forwardRequest(targetURIList, sipProvider, request, serverTransaction, false);
            ...
            if (registrar.hasRegistration(request))
            {
                ...
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("Proxy, processRequest(), the request "
                ...
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("Proxy, processRequest(), the target set"
                                + " is the set of the contacts URI from the " + " location service");
            ...
            ProxyDebug.println("Proxy, processRequest(), the registrar cannot help"
        ...
        } catch (Exception ex)
        {
            try
            {
                ...
                if (ProxyDebug.debug)
                    ProxyDebug.println("Proxy, processRequest()," + " 500 SERVER_INTERNAL_ERROR replied:\n"
                            + response.toString());
            } catch (Exception e)
            {
                e.printStackTrace();
    ...
    public void processResponse(ResponseEvent responseEvent)
    {
        try
        {
            ...
            ProxyDebug.println("\n***************************************************************"
                    + "\n***************************************************************" + "\nResponse "
                    + response.getStatusCode() + " " + response.getReasonPhrase() + " received:\n"
                    + response.toString());
    ...
    public void processTimeout(TimeoutEvent timeOutEvent)
    {
        ...
        } else
        {
            ...
            if (dialog != null)
            {
                ...
                {
                    ProxyDebug
                            .println("ERROR, Unable to retrieve the server transaction," + " cannot process timeout!");
                    return;
                }
            } else
            {
                ProxyDebug.println("ERROR, Unable to retrieve the transaction Mapping," + " cannot process timeout!");
                return;
            }
        ...
        if (configuration != null && configuration.isValidConfiguration())
        {
            ...
            if (configuration.enableDebug)
            {
                ...
                if (configuration.badMessageLogFile != null)
                    properties.setProperty("gov.nist.javax.sip.BAD_MESSAGE_LOG", configuration.badMessageLogFile);
            ...
            for (int i = 0; lps != null && i < lps.size(); i++)
            {
                ...
                try
                {
                    ...
                    ListeningPoint lp = sipStack.createListeningPoint(Integer.valueOf(a.port).intValue(), a.transport);
