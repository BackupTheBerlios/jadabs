...
import gov.nist.sip.proxy.ProxyUtilities;
...
/**
 * The PresenceServer stores presence info about those registered user agents
 * (presentities) that other user agents (watchers) are subscribing to. <br>
 * 
 * The PresenceServer processes Register, Publish, Subscribe and Notify
 * requests.
 * 
 * The PresenceServer filters incoming requests and if no obvious errors are
 * found, it pushes them down to the PresentityManager for further processing.
 * The PresentityManager returns a responseCode that is used to send a response
 * to the request.
 * 
 * <h3>ToDo</h3>
 * <ul>
 * <li>If a subscription for xpidf-data is received for a user that is
 * registered it is forwarded, but if this user, the notifier, is offline, the
 * subscription response will ultimately be a 404. instead, buffer the
 * subscription and send a 202.</li>
 * <li>An empty Notify is sent after the initial one when subscribing an
 * offline notifier. Remove it. </
 * </ul>
 * 
 * @author deruelle
 * @author Henrik Leion
...
public class PresenceServer
{
    ...
    /**
     * When a User Agent registers to the Registrar, the PresenceServer hands
     * over all the contact Uri:s with expires times to the presentityManager.
     * This method does not send a response. That is done by the registrar.
     * <p>
     * 
     * TODO:
     * <ul>
     ...
     * </ul>
     */
    public void processRegisterRequest(SipProvider sipProvider, Request request, ServerTransaction serverTransaction)
    {
        ...
        try
        {
            ...
            //We are acting as the EventStateCompositor and/or PresenceAgent
            // for
            ...
            {
                expires = getExpiresTime(); //This is the PresenceServer
                                            // expirestime
            } else
            {
            ...
            while (cit.hasNext())
            {
                ...
                /*
                 * Could do something about individual expires times. Note,
                 * there must still be the general expires time int localExpires =
                 * contactH.getExpires(); if(localExpires == -1) { localExpires =
                 * expires; }
                 */
     ...
     * on startup. The registrar calls this method to let the presenceserver
     * know of them so that other users may subscribe to them before they go
     * online.
     */
    ...
    public void processUploadedRegistration(Registration registration)
    {
         ...
         * java.lang.String getDisplayName() java.lang.String getKey()
         */
        ProxyDebug.println("PresenceServer.processUploadedRegistration: " + "\n   Key=" + registration.getKey()
                ...
                + registration.getContactsList().toString());
     ...
     */
    public void processPublishRequest(SipProvider sipProvider, Request request, ServerTransaction serverTransaction)
    {
     ...
     * 
     * The proxy has already verified that we are responsible for the domain and
     * that we are tha ESC or PA for the notifier, so there should be no need
     * for forwarding the request. <br>
     * 
     * 
     * <h3>Policy for forwarding subscriptions.</h3>
     * <ol>
     ...
     * according to authorization. Note, the default process should be that the
     * notifiers are all registered from the registrations.xml file, so they
     * don't have to be online.</li>
     * <li>If the notifier is registered, but the subscriber requires some
     * non-pidf notification format (such as Windows Messengers xpidf) or has
     * other special requirements that the PS can't understand, the subscription
     * is forwarded statelessly to notifyer.</li>
     ...
     * The subscriber must always know who is receiving his notiofications, and
     * must be able to terminate all notifications individually. There are three
     ...
     * impersonate subscriber. No other subscriber may utilize the notifications
     * received in this dialog, but the server may collect notifications from
     * several sources and send to subscriber</li>
    ...
    public void processSubscribeRequest(SipProvider sipProvider, Request request, ServerTransaction serverTransaction)
    {
        ...
        if (ProxyDebug.debug)
        {
            ProxyDebug.println("PresenceServer:processSubscribeRequest:received a request\n" + request);
        }
     ...
     * Subscriptions that the PresenceServer has issued to external resources on
     * behalf of a resource list. <br>
     * Updates the internal presence information. <br>
     * Checks that the attached pidf-document is valid, and replies with a 200
     * OK. Sets the subscribers dirty bit or terminates the serversubscription
     * depending on the subscription state.
     */

    public void processNotifyRequest(SipProvider sipProvider, Request notify, ServerTransaction serverTransaction)
    {
        if (ProxyDebug.debug)
        {
        ...
        try
        {
            ...
            ProxyDebug.println("processNotifyRequest(), response sent:" + response.toString());
    ...
    /**
     * If the response is not a class 2xx response, we might want to try again
     * or terminate all subscriptions to this UA. Otherwise hand over the
     * recently created dialog to the presentityManager.
     */
     ...
     *  
     */
    protected void sendSubscribeRequest(String toURI, int expires, Dialog dialog)
    {
    ...
    protected void sendNotifyRequest(Subscriber subscriber)
    {
        ...
        if (ProxyDebug.debug)
        {
            ProxyDebug.println("PresenceServer:sendNotifyRequest to " + subscriber.getSubscriberURL());
        }
        try
        {
            ...
            if (notifyBody == null || notifyBody.equals(""))
            {
                ...
                {
                    ProxyDebug.println("PresenceServer:sendNotifyRequest: " + "   NotifyBody is null \n"
                    ...
                    || (subscriptionState.equalsIgnoreCase(SubscriptionStateHeader.PENDING)))
            {
                ...
                //The media type should be derived from the Registration
                // request
            ...
            {
                ProxyDebug.println("subscription state is " + subscriptionState);
            }
    ...
    /**
     * returns true if the notifier of this subscription is publishing it's
     * state here and if the subscriber understands pidf+xml (modify if the
     * server starts handling new formats) <h3ToDo</h3>
     * <ul>
     * <li>Should not only check if the notifier is registered, but if he
     * actually is publishing his state info, instead of assuming it. This means
     * that the registrations in registrations.xml must indicate this.</li>
     * </ul>
     */
    public boolean isStateAgent(Request request)
    {
        ...
        {
            ProxyDebug.println("could not find notifier record for " + notifierKey);
            return false;
        }
        return false;
    }
    ...
    /**
     * Returns true if the registration records the from header as being a
     * buddy. So OK to report that buddy has not yet registered.
     */
    public boolean isBuddy(Request request)
    {
        ...
        if (proxy.getRegistrar().hasRegistration(notifierKey))
        {
            ...
            if (registration.isMyBuddy(subscriberKey))
                return true;
            else
                return false;
        } else
    ...
    /**
     * This is only here because RequestValidation.java and the Registrar needs
     * it. Should deprecate.
     * 
