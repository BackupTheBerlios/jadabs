...
import gov.nist.sip.proxy.registrar.Domain;
...
public class ProxyConfigurationHandler extends DefaultHandler
{
    ...
    public void endDocument() throws SAXException
    {
        try
        {
            ProxyDebug.println("The XML configuration file has been successfully parsed!");
            ProxyDebug.println("---------------------------------------");
        } catch (Exception e)
        {
            throw new SAXException("XMLRegistrationsParser error", e);
            ...
            Attributes attrs) throws SAXException
    {
        ...
        if (element.compareToIgnoreCase("SIP_STACK") == 0)
        {
            ...
            } else
            {
                ...
                configurationMessage("INFORMATION: the name of the stack is set to:" + configuration.stackName);
            }

            String stackIPAddress = attrs.getValue("stack_IP_address");
            if (stackIPAddress == null || stackIPAddress.trim().equals(""))
            {
                configurationMessage("ERROR: the IP address of the stack is not set");

            } else
            {
                configuration.stackIPAddress = stackIPAddress.trim();
                configurationMessage("INFORMATION: the stack IP address is set to:" + stackIPAddress);
            }

            String outboundProxy = attrs.getValue("outbound_proxy");
            if (outboundProxy == null || outboundProxy.trim().equals(""))
            {
                configurationMessage("INFORMATION: the outbound proxy is not set");
            } else
            {
                configuration.outboundProxy = outboundProxy.trim();
                configurationMessage("INFORMATION: the outbound proxy is " + outboundProxy);
            }

            String routerPath = attrs.getValue("router_path");
            if (routerPath == null || routerPath.trim().equals(""))
            {
                configurationMessage("WARNING: the path for the router is not set using default");
            } else
            {
                configuration.routerPath = routerPath.trim();
                configurationMessage("INFORMATION: the path for the router is set to: " + routerPath.trim());
            }

            String extensionMethods = attrs.getValue("extension_methods");
            if (extensionMethods == null || extensionMethods.trim().equals(""))
            {
                configurationMessage("INFORMATION: no extensions are supported");
            } else
            {
                configuration.extensionMethods = extensionMethods.trim();
                configurationMessage("INFORMATION: extensions supported: " + extensionMethods);
            }
            ...
            {
                configurationMessage("INFORMATION: the retransmission filter is set to:" + retransmissionFilter);
                configuration.retransmissionFilter = retransmissionFilter.trim();
            } else
                configurationMessage("INFORMATION: no default retransmission filter");

            String stopTime = attrs.getValue("stop_after");
            if (stopTime != null && !stopTime.trim().equals(""))
            {
                configurationMessage("INFORMATION: The proxy will stop automatically after " + stopTime + " ms");
            ...
            {
                configurationMessage("INFORMATION: The proxy will authorize only " + maxConnections + " connections");
            ...
            {
                configurationMessage("INFORMATION: The proxy will authorize only " + maxServerTransactions
                        + " server transactions");
            ...
            {
                configurationMessage("INFORMATION: The thread pool size is set to " + threadPoolSize);
        ...
        if (element.compareToIgnoreCase("LISTENING_POINT") == 0)
        {
            ...
            {
                configurationMessage("ERROR: the port of the stack is not set");
            ...
            } else
            {
                ...
                {
                    configurationMessage("ERROR: the transport is not set");

                } else
                {
                    configurationMessage("LISTENING_POINT port = " + stackPort);
                    configurationMessage("LISTENING_POINT transport = " + stackTransport);
        ...
        if (element.compareToIgnoreCase("DOMAIN") == 0)
        {
            ...
            {
                configurationMessage("INFORMATION: One domain to take care of is: " + stackDomain);
        ...
        if (element.compareToIgnoreCase("REGISTER_TO_PROXY") == 0)
        {
            ...
            if (hostName != null && !hostName.trim().equals(""))
            {
                ...
                if (hostPort != null && !hostPort.trim().equals(""))
                {
                    ...
                    configurationMessage("INFORMATION: One proxy to register with: " + hostName + ":" + hostPort);
                } else
                    configurationMessage("INFORMATION: One proxy to register with: " + hostName);
        ...
        if (element.compareToIgnoreCase("REGISTRAR") == 0)
        {
            ...
            if (rfc2543Compatible != null && "true".equalsIgnoreCase(rfc2543Compatible.trim()))
            {
            ...
            if (exportRegistry != null && !exportRegistry.trim().equals(""))
            {
                try
                {
                    ...
                    if (b.booleanValue())
                    {
                        ...
                        configurationMessage("INFORMATION: the registrations will be exported");
                        configuration.exportRegistry = true;
                        String registryPort = attrs.getValue("port");
                        if (registryPort == null)
                        {

                            configurationMessage("WARNING: The registry port is not set; " + "1099 is used by default");

                            configuration.registryPort = "1099";
                        } else
                        {

                            configurationMessage("INFORMATION: The registry port is: " + registryPort);
                            configuration.registryPort = registryPort;
                        }
                    } else
                    {

                        configurationMessage("INFORMATION: the registrations will not be exported");
        ...
        if (element.compareToIgnoreCase("PRESENCE_SERVER") == 0)
        {
            ...
            if (enablePresenceServerString == null)
            {
                ...
                configurationMessage("WARNING: The presence server feature is disabled");
            } else
            {
                try
                {
                    Boolean b = Boolean.valueOf(enablePresenceServerString.trim());
                    if (b.booleanValue())
                    {
                        configuration.enablePresenceServer = true;
                        configurationMessage("INFORMATION: The presence server feature is enabled");
                    } else
                    {
                        configuration.enablePresenceServer = false;
                        configurationMessage("WARNING: The presence server feature is disabled");
        ...
        if (element.compareToIgnoreCase("AUTHENTICATION") == 0)
        {
            ...
            if (enableAuthenticationString == null)
            {
                ...
                configurationMessage("WARNING: Authentication support is disabled");
            } else
            {
                try
                {
                    Boolean b = Boolean.valueOf(enableAuthenticationString.trim());
                    if (b.booleanValue())
                    {
                        configuration.enableAuthentication = true;
                        configurationMessage("INFORMATION: Authentication feature is enabled");

                    } else
                    {
                        configuration.enableAuthentication = false;
                        configurationMessage("WARNING: The authentication feature is disabled");
                    }
                    String method = attrs.getValue("method");
                    if (method == null)
                    {
                        configurationMessage("WARNING: The Authentication method is not set");
                    } else
                    {
                    ...
                    if (classFile == null)
                    {
                        configurationMessage("WARNING: The class_file parameter is not set" + " for the authentication");
                    } else
                    {
                    ...
                    {
                        configurationMessage("WARNING: The passwords file is not set" + " for the authentication");
                    } else
                    {
                ...
                {
                    configuration.enableAuthentication = false;
                    configurationMessage("ERROR: Error initializing authentication [" + e.getMessage() + "]");
                    System.exit(0);
                }
            }
        }
        ...
        {
            String remoteLogAccess = attrs.getValue("access_log_via_rmi");

            if (remoteLogAccess != null && remoteLogAccess.equalsIgnoreCase("true"))
            {
                configurationMessage("INFORMATION: RMI access to log file enabled");
                configuration.accessLogViaRMI = true;
                String rmiPort = attrs.getValue("rmi_port");
                if (rmiPort != null)
                {
                    configuration.logRMIPort = rmiPort;
                    configurationMessage("INFORMATION: the RMI port for log file is: " + configuration.logRMIPort);
                } else
                {
                    configuration.logRMIPort = "0";
                }
                configurationMessage("INFORMATION: the default RMI port for log file is: " + configuration.logRMIPort);

                configuration.logLifetime = attrs.getValue("log_lifetime");
                if (configuration.logLifetime != null)
                    configurationMessage("INFORMATION: the log lifetime is: " + configuration.logLifetime);
            } else
            {
                configurationMessage("INFORMATION: RMI access to log file disabled");
            }
            ...
            if (enableDebug == null || enableDebug.trim().equals(""))
            {
                ...
                configurationMessage("INFORMATION: the Debug feature is not set");
            } else
            {
                try
                {
                    boolean deb = Boolean.valueOf(enableDebug.trim()).booleanValue();
                    if (deb)
                    {
                        configurationMessage("INFORMATION: the Debug feature is set");
            ...
            {
                configurationMessage("INFORMATION: the server message log file is not set");
            } else
            {
                configuration.serverLogFile = serverLogFile.trim();
                configurationMessage("INFORMATION: the server message log file is: " + configuration.serverLogFile);
            }
            String badMessageLog = attrs.getValue("bad_message_log");
            if (badMessageLog == null || badMessageLog.trim().equals(""))
            {
                configurationMessage("INFORMATION: the bad message log file is not set");
            } else
            {
                configuration.badMessageLogFile = badMessageLog.trim();
                configurationMessage("INFORMATION: the bad message log is: " + configuration.badMessageLogFile);
            }
            String debugLog = attrs.getValue("debug_log");
            if (debugLog == null || debugLog.trim().equals(""))
            {
                configurationMessage("INFORMATION: the debug log file is not set");
            } else
            {
                configuration.debugLogFile = debugLog.trim();
                configurationMessage("INFORMATION: the debug log file is: " + configuration.debugLogFile);
            }
            String outputProxy = attrs.getValue("output_proxy");
            if (outputProxy == null || outputProxy.trim().equals(""))
            {
                configurationMessage("INFORMATION: the output proxy file is not set");
            } else
            {
                configuration.outputProxy = outputProxy.trim();
                configurationMessage("INFORMATION: the output proxy file is: " + configuration.outputProxy);
            }
        }
        if (element.compareToIgnoreCase("REGISTRATIONS") == 0)
        {
            ...
            if (expiresTimeString != null && !expiresTimeString.trim().equals(""))
            {
                try
                {
                    configuration.expiresTime = Integer.valueOf(expiresTimeString.trim()).intValue();
                    configurationMessage("INFORMATION: expires time is set to:" + configuration.expiresTime);
                } catch (Exception e)
                {
                    configurationMessage("WARNING: expires time is set to default value:" + configuration.expiresTime);
                }
            } else
            {
                configurationMessage("WARNING: expires time is set to default value:" + configuration.expiresTime);
            ...
            if (enableRegistrationsString == null)
            {
                ...
                configurationMessage("INFORMATION: registration uploading feature is disabled");
            } else
            {
                try
                {
                    ...
                    if (b.booleanValue())
                    {
                        ...
                        configurationMessage("INFORMATION: Uploading static registrations");
                    } else
                    {
                        configuration.enableRegistrations = false;
                        configurationMessage("WARNING: No static registrations to upload");
                    }
                    String registrationsFile = attrs.getValue("registrations_file");
                    if (registrationsFile == null)
                    {
                        configurationMessage("WARNING: No registration file specified");
                    } else
                    {
                        configurationMessage("INFORMATION: registrations file is " + registrationsFile);
                        configuration.registrationsFile = registrationsFile.trim();
                    }

                } catch (Exception e)
                {
                    configuration.enableRegistrations = false;
                    configurationMessage("ERROR: Error uploading static registrations [" + e.getMessage() + "]");
                }
            }
        }
    ...
    public static void writeFile(String outFile, String text)
    {
        try
        {
            ...
            if (file.exists())
            {
                ...
                ProxyDebug.println("ProxyDebug, writeFile(),  unable to write, the file:" + outFile + " is missing.");
        } catch (Exception e)
        {
            ProxyDebug.println("ProxyDebug, writeFile(),  unable to write, the file:" + outFile + " is missing.");
            e.printStackTrace();
