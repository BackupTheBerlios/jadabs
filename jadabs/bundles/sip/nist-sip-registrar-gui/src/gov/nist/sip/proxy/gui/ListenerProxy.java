...
public class ListenerProxy
{
    ...
    public void startRMIregistry()
    {
        ...
        try
        {
            ...
            String rmiregistryClasspath = localRootDirectory + "/classes" + localSeparator + localRootDirectory
                    + "/lib/antlr/antlrall.jar" + localSeparator + localRootDirectory + "/lib/xerces/xerces.jar";
...
//                    registrar.setRegistrationsList(proxyLauncher.registrationsList);
                ...
                {
                    new AlertFrame("ERROR: the configuration parameters are not correct!", JOptionPane.ERROR_MESSAGE);
                }
                ProxyDebug.println("\n*****************************************************\n");
            } else
                stopProxy();
    ...
    public void stopProxy()
    {
        try
        {
            ...
            //                } catch (Exception e)
            //                {
            //                    ProxyDebug.println("DEBUG, traces viewer" + " not closed,
            // exception raised:");
            //                    e.printStackTrace();
            //                }
            //            }
    ...
    public void traceViewerActionPerformed(ActionEvent evt)
    {
        try
        {
            ...
            {
                new AlertFrame("ERROR: Configure the proxy before viewing the traces!", JOptionPane.ERROR_MESSAGE);
                return;
            }

            if (configuration.stackIPAddress == null)
            {
                new AlertFrame("ERROR: Specify the stack IP address!", JOptionPane.ERROR_MESSAGE);
                return;
            }
            /*
             * String back="./tools/tracesviewer/images/back.gif"; String
             * faces="../../tools/tracesviewer/images/faces.jpg"; String
             * actors="../../tools/tracesviewer/images/comp.gif"; String
             * logoNist="../../tools/tracesviewer/images/nistBanner.jpg";
             */
            ...
            //                tracesViewer = new TracesViewer(configuration.serverLogFile,
            // traces, parser.logName,
            //                        parser.logDescription, parser.auxInfo, "Proxy Traces Viewer",
            // back, faces, actors, logoNist);
            //                tracesViewer.show();
            //            } else
            if (configuration.serverLogFile == null)
            {
                new AlertFrame("ERROR: Specify a server log file before viewing the traces!", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (Exception e)
        {
